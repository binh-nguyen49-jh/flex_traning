...
import css from './HoursFilterPopup.module.css';
...
class HoursFilterPopup extends Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    const initialHours =
      initialValues && initialValues[hoursQueryParam] ? parse(initialValues[hoursQueryParam]) : {};
    ...
    const currentLabel = hasInitialValues
      ? intl.formatMessage(
          { id: 'HoursFilter.labelSelectedButton' },
          {
            minHours: formatCurrencyMajorUnit(intl, currencyConfig.currency, minHours),
            maxHours: formatCurrencyMajorUnit(intl, currencyConfig.currency, maxHours),
          }
        )
      : label
      ? label
...
export default injectIntl(HoursFilterPopup);
