...
const tabLabel = (intl, tab) => {
  ...
  if (tab === GENERAL) {
    ...
    key = 'EditProgramWizard.tabLabelPricing';
  } else if (tab === AVAILABILITY) {
    key = 'EditProgramWizard.tabLabelAvailability';
  } else if (tab === PHOTOS) {
    key = 'EditProgramWizard.tabLabelPhotos';
  }
...
const createReturnURL = (returnURLType, rootURL, routes, pathParams) => {
  const path = createResourceLocatorString(
    'EditProgramStripeOnboardingPage',
    routes,
...
const RedirectToStripe = ({ redirectFn }) => {
  ...
  return <FormattedMessage id="EditProgramWizard.redirectingToStripe" />;
};

// Create a new or edit listing through EditProgramWizard
class EditProgramWizard extends Component {
  constructor(props) {
    super(props);
  ...
  handlePayoutSubmit(values) {
    this.props
      ...
      .then(response => {
        this.props.onManageDisableScrolling('EditProgramWizard.payoutModal', false);
      })
      .catch(() => {
        // do nothing
      });
  ...
  render() {
    ...
    return (
      <div className={classes}>
        ...
        <Modal
          id="EditProgramWizard.payoutModal"
        ...
        >
          <div className={css.modalPayoutDetailsWrapper}>
            <h1 className={css.modalTitle}>
              <FormattedMessage id="EditProgramWizard.payoutModalTitleOneMoreThing" />
              <br />
              <FormattedMessage id="EditProgramWizard.payoutModalTitlePayoutPreferences" />
            </h1>
            {!currentUserLoaded ? (
              ...
              <>
                <p className={css.modalMessage}>
                  <FormattedMessage id="EditProgramWizard.payoutModalInfo" />
                </p>
...
EditProgramWizard.defaultProps = {
  className: null,
  currentUser: null,
  rootClassName: null,
  listing: null,
